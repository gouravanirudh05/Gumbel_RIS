...
# ==========================================================
S_MATRIX_PATH = f"S_matrix_N{Nris}_lambda_4.npy"
...
caps_std_std = []   # optimized without MC, evaluated without MC
caps_std_mc  = []   # optimized without MC, evaluated WITH MC
caps_mc_std  = []   # optimized with MC,    evaluated without MC
caps_mc_mc   = []   # optimized with MC,    evaluated WITH MC
times_std = []
times_mc = []
...
for i in tqdm(range(N_EVAL)):
    ...
    caps_std_std.append(compute_capacity(C_no_mc))
    ...
    caps_std_mc.append(compute_capacity(C_with_mc))
    ...
    caps_mc_std.append(compute_capacity(C_no_mc2))
    # Eval WITH MC (what actually happens in real life)
    SPhi2 = S_torch @ Phi_mc
    Phi_eff_mc = Phi_mc @ torch.linalg.inv(I_N - SPhi2)
    C_with_mc2 = G @ Phi_eff_mc @ H + D
    caps_mc_mc.append(compute_capacity(C_with_mc2))

# ==========================================================
# RESULTS — 2×2 GRID (same format as GumbelRIS)
# ==========================================================
m_std_std = np.mean(caps_std_std)
m_std_mc  = np.mean(caps_std_mc)
m_mc_std  = np.mean(caps_mc_std)
m_mc_mc   = np.mean(caps_mc_mc)
...
print(f"\nMC gain (MC-aware vs Std, eval WITH MC — the real-world gain):")
print(f"  {m_mc_mc:.4f} - {m_std_mc:.4f} = {mc_gain_abs:.4f} bps/Hz ({mc_gain_pct:.2f}%)")
print(f"\nTiming:")
